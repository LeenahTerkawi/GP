...
var game = {
  ...
  setHandlers: function() {
    ...
    $('#next').on('click', function() {
      ...
      $('.bird').addClass('animated bounceOutUp');
    ...
    $('#colorblind').on('change', function() {
      ...
      if (game.colorblind == 'true') {
        $('.birdhouse, .bird').addClass('cb-friendly');
      } else {
        $('.birdhouse, .bird').removeClass('cb-friendly');
      }
    });
  ...
  loadLevel: function(level) {
    $('#editor').show();
    $('#share').hide();
    $('#background, #sky').removeClass('wrap').attr('style', '').empty();
    $('#levelsWrapper').hide();
    ...
    for (var i = 0; i < string.length; i++) {
      ...
      var birdhouse = $('<div/>').addClass('birdhouse ' + color + (this.colorblind == 'true' ? ' cb-friendly' : '')).data('color', color);
      var bird = $('<div/>').addClass('bird ' + color + (this.colorblind == 'true' ? ' cb-friendly' : '')).data('color', color);

      $('<div/>').addClass('bg').css(game.transform()).appendTo(birdhouse);
      $('<div/>').addClass('bg animated pulse infinite').appendTo(bird);

      $('#background').append(birdhouse);
      $('#sky').append(bird);
    }
  ...
  win: function() {
    ...
    $('#code').val(solution);
    $('#share').show();
    $('.bird .bg').removeClass('pulse').addClass('bounce');
  },
